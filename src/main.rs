...
fn handle_future_date(args: &[String]) {
    if args.is_empty() {
        println!("❌ Please specify number of days");
    ...
    if days.is_none() {
        println!("❌ Could not parse days. Use format like: 69 days, 69d, or just 69");
        return;
    }
